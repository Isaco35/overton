...
const KINGS = [
  {
    ...
    note:"Charismatic authority — movement-dependent, not institutional. Governance record (rent freeze ✓, free buses ✗) read as irrelevant in the Zomentum frame — a framing now constrained by Falsification register F1–F4 (Methodology). NOTE: he has endorsed no one outside NYC; the 'nationwide' claim in the win-branch is a scenario projection, not a current fact.",
...
const NODES = [
  ...
  },
  {
    ...
    detail:"Francesca Hong ({DS}, 37, Korean American, single mother, chef, third-term Madison Assembly rep) vs Mandela Barnes (D, fmr Lt. Gov.), Sara Rodriguez (D, Lt. Gov.), David Crowley (D), Kelda Roys (D), Joel Brennan (D); Missy Hughes suspended June, endorsed Rodriguez. Feb Marquette: Hong 11%, Barnes 10%, 65% undecided. May poll (commissioned by Rodriguez campaign + aligned org): Barnes 26%, Hong 22%, Rodriguez 15% — Hong up from 14% in March. Convention straw poll Jun 14: Rodriguez 27.5%, Hong 23.1%. Campaign-reported (not independently audited): 5,548 event participants across 62 of 72 counties, 8,000+ donors, no corporate PAC money. Attack vectors live: police abolition posts 2020–21 (not deleted, defended — CNN KFile); Capital One $30k debt suit — paid, dismissed. Hong appeared on Hasan Piker's stream Jun 29, ~$57k raised.",
    ...
    <div className="wrap clear">
      ...
      {tab==="overview" && (
        <div>
          ...
          <h2>Zomentum</h2>
          <p>Zomentum is the gravitational pull of Zohran Mamdani's 2025 NYC mayoral victory on every subsequent {"{DS}"} candidate — a Harrow construct, not a measured force. Grounded decomposition (verified Jul 2, 2026): <b>(1) Direct endorsement effect — real but strictly local.</b> Mamdani endorsed three NYC congressional challengers and went 3-for-3 against the establishment, unseating two incumbents with Jeffries campaigning on the losing side. FACT: as of Jul 2, 2026, he has endorsed no candidate outside New York City (Ballotpedia endorsement ledger). Kiros ran on Sanders' endorsement, not his. He has stayed in his lane. <b>(2) Infrastructure export.</b> What actually traveled to Colorado was not Mamdani's word — it was DSA/Justice Democrats machinery, Sanders' network, and small-dollar energy. <b>(3) Narrative pull.</b> The press independently frames Colorado, Wisconsin, and Michigan as tests of "his" momentum, which makes the construct partly self-fulfilling and hard to separate from (2). <b>(4) Brand drag — the counter-force.</b> Republicans are running Mamdani-tie attacks against Democrats in Michigan and Colorado; outside deep-blue terrain, the same name is ammunition. In the physics model, Zomentum is the coupling variable that determines whether ANGRY and GUIDED amplify together (compounding acceleration) or dissipate (friction). His unmet promises (free buses) are read here as narrative proof the system is rigged, his delivered wins (rent freeze) as a credibility floor — a framing the Falsification register now constrains.</p>
          <div className="block warn">
            <span className="lab">Falsifiability — conditions committed 2026-07-02</span>
            The old version of this warning said Zomentum could absorb any outcome and that no pre-committed failure condition existed. That is no longer true: four conditions (F1–F4) are now logged in Methodology → Falsification register, dated before any August result exists. If a condition fires, the part of the construct it names dies — no post-hoc rescue, no reinterpretation. If we rescue it anyway, that act itself is logged as model failure. The construct remains a construct; what changed is that it can now lose.
          </div>
      ...
      {tab==="methodology" && (
        <div>
          ...
          <p>Zomentum tracks the gravitational pull of Mamdani's NYC mayoralty on subsequent {"{DS}"} candidates. When Zomentum is high, ANGRY and GUIDED tend to align (compounding mode). When Zomentum collapses, they decouple (friction or stacking mode). Each node carries a Zomentum delta. As of 2026-07-02 this variable carries pre-committed failure conditions — see the Falsification register directly below. Before that date it was not built to be provably wrong, which failed the model's own bar for trustworthiness.</p>
          <h3>Falsification register</h3>
          <p>Committed July 2, 2026 — before any August result exists. Rules of the register: a fired condition kills the claim it names. Explaining the result away after the fact is prohibited; if it happens anyway, that act is logged in the public dev log as model failure. Conditions are only added or amended <i>before</i> the events they reference.</p>
          <table style={{width:"100%", borderCollapse:"collapse", fontSize:".85em"}}>
            <thead><tr>
              <th style={{padding:"5px 7px", border:"1px solid var(--line)", textAlign:"left"}}>ID</th>
              <th style={{padding:"5px 7px", border:"1px solid var(--line)", textAlign:"left"}}>Condition (trigger)</th>
              <th style={{padding:"5px 7px", border:"1px solid var(--line)", textAlign:"left"}}>What dies if it fires</th>
            </tr></thead>
            <tbody>
              <tr>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>F1</td>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>Hong finishes <b>third or worse</b> in the Wisconsin primary, Aug 11.</td>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>The claim that Zomentum transfers to statewide races. Zomentum deltas zero out for all statewide-tier nodes; Scenario A/E "scales statewide" language is struck.</td>
              </tr>
              <tr>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>F2</td>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>{"{DS}"} goes <b>0-for-3</b> in the August congressionals (MO-01, MI-13, FL-25).</td>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>The "repeatable infrastructure" claim. Compounding mode is disabled until two consecutive {"{DS}"} forward-node wins occur.</td>
              </tr>
              <tr>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>F3</td>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>El-Sayed <b>loses</b> the Michigan Senate primary, Aug 4. (He enters it leading by ~7; a loss from ahead is the clean version of this test — if polls tighten first, the signal is weaker and gets logged as such, but the condition still fires.)</td>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>The "Senate structural power" tier. michigan_senate loses β-node status; Zomentum is capped at the congressional tier.</td>
              </tr>
              <tr>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>F4</td>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>In the November generals, {"{DS}"} nominees underperform the generic-Democrat margin in a majority of swing districts where Republicans ran Mamdani-tie ads.</td>
                <td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>The claim that Zomentum is net-positive outside safe seats. Its sign flips negative for general-election nodes; Scenario C (Backlash) weighting is elevated.</td>
              </tr>
            </tbody>
          </table>
          <p style={{fontSize:".85em", color:"#9ca3af"}}>Register status: F1–F4 cover Zomentum only. Model-level falsifiers (physics, scenario coverage, psyche coupling) are being drafted next and will be added here before the events they reference. An empty row in this table is more honest than a rescued theory.</p>
          <h3>Scale</h3>
          ...
          <table className="wt" style={{width:"100%", borderCollapse:"collapse", fontSize:".85em", margin:"10px 0"}}>
            <tbody>
              ...
              <tr><td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>Unfalsifiable construct</td><td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>Zomentum explains both delivery and non-delivery as confirming evidence</td><td style={{padding:"5px 7px", border:"1px solid var(--line)"}}>RESOLVED 2026-07-02: pre-committed failure conditions F1–F4 added to the Falsification register, dated before the events they reference. Also corrected: Mamdani has endorsed no one outside NYC — the "borrowed credibility" mechanism was overstated</td></tr>
